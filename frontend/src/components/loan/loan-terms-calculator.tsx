...
const TENURE_OPTIONS = [
  { months: 6, label: '6 Months', tag: 'Fast Payoff', badgeColor: 'bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300' },
  { months: 12, label: '12 Months (1 Yr)', tag: 'Popular', badgeColor: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300' },
  { months: 18, label: '18 Months', tag: 'Balanced', badgeColor: 'bg-teal-100 text-teal-800 dark:bg-teal-950/60 dark:text-teal-300' },
  { months: 24, label: '24 Months (2 Yrs)', tag: 'Recommended', badgeColor: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-950/60 dark:text-indigo-300' },
  { months: 36, label: '36 Months (3 Yrs)', tag: 'Lowest EMI', badgeColor: 'bg-purple-100 text-purple-800 dark:bg-purple-950/60 dark:text-purple-300' },
];
...
export function LoanTermsCalculator({ onSuccess }: LoanTermsCalculatorProps) {
      ...
      try {
        ...
        if (res.data) {
          ...
          if (res.data.currentTerms) {
            ...
            setAmount(Number(current.amount) || Math.min(300000, maxLimit));
            setTenureMonths(Number(current.tenureMonths) || 24);
          } else {
    ...
    async (targetAmount: number, targetTenure: number) => {
      ...
      try {
        ...
        if (err instanceof ApiError) {
          ...
          setErrorMessage('Could not update calculations. Please check connection.');
        }
      } finally {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        setErrorMessage('Failed to confirm loan terms. Please try again.');
      }
    } finally {
  ...
  return (
    <div className="space-y-6">
      {/* Notifications */}
      {errorMessage && (
        <div className="flex items-start space-x-2 rounded-xl border border-rose-200 bg-rose-50 p-3.5 text-xs text-rose-800 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-300">
      ...
      {successMessage && (
        <div className="flex items-start space-x-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3.5 text-xs text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-300">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="space-y-6 lg:col-span-7">
          <Card className="border-slate-200/80 shadow-glass">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3 rounded-2xl border border-slate-200/70 bg-slate-50/50 p-4 dark:border-slate-800/80 dark:bg-slate-900/40">
                <div className="flex items-center justify-between">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-xl border border-slate-300 bg-white pl-8 pr-4 text-xl font-extrabold text-slate-900 transition-all focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                  />
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`group relative flex flex-col items-center justify-center rounded-2xl border p-3 transition-all ${
                          isSelected
                            ? 'border-emerald-600 bg-emerald-50/70 text-emerald-950 shadow-md ring-2 ring-emerald-500/20 dark:border-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-200'
                            : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50/80 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-slate-700'
                        }`}
                      >
                        <span className={`mb-1 inline-block rounded-full px-2 py-0.5 text-[9px] font-bold ${opt.badgeColor}`}>
                          {opt.tag}
                        </span>
                        <span className="text-sm font-extrabold">{opt.months} Mo</span>
              ...
              <div className="space-y-2 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4 dark:border-slate-800/80 dark:bg-slate-900/40">
        ...
        <div className="space-y-6 lg:col-span-5">
          <Card className="relative overflow-hidden border-emerald-600/30 bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950 text-white shadow-xl">
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-emerald-400">
                  Calculated Equated Monthly Payment
                </span>
                {isCalculating && (
            ...
            <CardContent className="space-y-4 pt-2">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-xl border border-slate-800/80 bg-slate-800/40 p-3">
                  <div className="text-[11px] font-medium text-slate-400">Annual Interest Rate</div>
                  <div className="mt-1 text-lg font-bold text-white">
                    {breakdown ? `${breakdown.interestRate}%` : '--'}
                  </div>
                  <div className="text-[10px] text-slate-500">Fixed Flat/Reducing</div>
                </div>
                ...
                <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/30 p-3">
                  ...
                  <div className="text-[10px] text-emerald-500/80">Newton-Raphson solved</div>
                </div>
              </div>
              ...
              <div className="space-y-2 rounded-xl border border-slate-800/80 bg-slate-900/60 p-3 text-xs">
                ...
                <div className="flex items-center justify-between rounded-lg bg-emerald-950/40 p-2 font-bold text-emerald-300">
                  <div className="flex items-center space-x-1.5">
              ...
              <div className="space-y-1.5 rounded-xl border border-slate-800/80 bg-slate-900/60 p-3 text-xs">
                <div className="flex items-center justify-between text-slate-400">
                  ...
                  <span className="text-slate-200">{formatCurrency(breakdown?.totalInterest || 0)}</span>
                </div>
                <div className="flex items-center justify-between font-bold text-slate-200">
                  <span>Total Repayment Amount</span>
                  <span className="text-emerald-400">{formatCurrency(breakdown?.totalRepayment || 0)}</span>
                </div>
              </div>
            </CardContent>
            ...
            <CardFooter className="pt-2">
              <Button
                ...
                className="h-12 w-full rounded-xl bg-gradient-to-r from-teal-500 to-emerald-500 font-bold text-slate-950 shadow-lg shadow-emerald-500/20 hover:from-teal-400 hover:to-emerald-400 active:scale-[0.99] disabled:opacity-50"
              >
                {isConfirming ? (
                  <>
